...
summary = df %>% group_by(pindex,method) %>%
  summarize(N1 = sum(stimulus == 1),
            ...
            mean_length_1 = mean(log10(zero_to_na(length[stimulus == 1])),
                                 na.rm=T),
            mean_length_2 = mean(log10(zero_to_na(length[stimulus > 1])),
                                 na.rm=T),
            W = W(log10(length)),
            N = sum(stimulus > -1),
            mean_ratio = mean_length_1 - mean_length_2,
            sd_length_1 = sd(log10(zero_to_na(length[stimulus == 1])),na.rm=T),
            sd_length_2 = sd(log10(zero_to_na(length[stimulus > 1])),na.rm=T),
...
strength_plot = function(df,uselog=T){
  ...
  ggplot(pdf,aes(x = log10(c_m),y=log10(c_a),fill=value)) +
    ...
    scale_x_continuous(name="Mutual Inhibition",breaks=log10(m_breaks),
                       labels=m_breaks) +
    scale_y_continuous(name="Adaptation",breaks=log10(a_breaks),
                       labels=a_breaks) +
    if (uselog) {
...
save_plot(file.path(dir,"absolute_N_strengths.pdf"),p,nrow=2,ncol=5,
          base_width=2,base_height=2.5)
...
p = p + ggtitle("Strengths - Log Ratio of Counts")
save_plot(file.path(dir,"count_strengths_peaks.pdf"),p,nrow=1,ncol=5,
          base_width=2,base_height=2.5)
...
save_plot(file.path(dir,"count_time_constants.pdf"),p,nrow=2,ncol=2,
