...
function run_unwrapped(cm::CorticalModel,s_in::Matrix;progressbar=true,
                       n_phases=8)
  s = s_in
  ...
  cr = zeros(eltype(s), size(s,1), N_r, N_s, n_phases, size(s,2))
  rmin,rmax = extrema(abs.(rates))
  smin,smax = extrema(scales)
  progress = Progress(length(rates)*length(scales),
                      desc="Cortical Simulation: ",
  ...
  for (pi,phase) in enumerate(linspace(-π,π,n_phases+1)[1:end-1])
    for (ri,rate) in enumerate(rates)
      z_t = if isnan(rate)
        # do not filter by rate
        (t_ifft * S)[1:size(s,1),:]
      ...
      for (si,scale) in enumerate(scales)
        if isnan(scale)
          ...
          # inefficient, but will get gaurantee my tests
          H = HR .* HS'
          h = ifft(H)
          h .= real.(h .* exp(phase*im))
          H = fft(h)

          z = ifft(S.*H)

			    cr[:, ri, si, pi, :] = real.(view(z,indices(s)...))
        end
...
function run_unwrapped2(cm::CorticalModel,s_in::Matrix;progressbar=true,
                        n_phases=8)
  s = s_in
    ...
    for (ri,rate) in enumerate(rates)
      ...
      for (si,scale) in enumerate(scales)
        if isnan(scale)
          ...
          z = ifft(S.*H)
          z .= real.(z .* exp(phase*im))

			    cr[:, ri, si, pi, :] = (view(z,indices(s)...))
        end
