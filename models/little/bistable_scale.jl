...
params = AdaptMI(c_m=1,τ_m=200ms,W_m=scale_weighting2(cs,0.5),
...
p = rplot(csa)
R"""
ggsave($(joinpath(dir,"2_scale_adaptmi.pdf")),$p)
"""

cohere = CoherenceModel(AuditoryModel.Params(cs),3,window=200ms,method=:nmf,
                        delta=150ms,maxiter=200,tol=1e-3)
crsa = cortical(csa[0s .. 10s,:,250Hz .. 1kHz];rates=(2.0.^(1:5))Hz)
tc = cohere(crsa)

p = rplot(cohere,tc,crsa)
R"""
ggsave($(joinpath(dir,"3_nmf_scale_adaptmi.pdf")),$p)
"""
