...
x_a = ab(120ms,120ms,1,10,500Hz,6,:without_a) |> normpower |> amplify(-10dB)
x_b = ab(120ms,120ms,1,10,500Hz,6,:without_b) |> normpower |> amplify(-10dB)
...
sp12 = ab(120ms,120ms,1,10,500Hz,12) |> normpower |> amplify(-10dB) |> spect
C12 = tempc(sp12)
...
sp1 = ab(120ms,120ms,1,10,500Hz,1) |> normpower |> amplify(-10dB) |> spect
C1 = tempc(sp1)
...
sp60 = ab(60ms,60ms,1,20,500Hz,6) |> normpower |> amplify(-10dB) |> spect
C60 = tempc(sp60)
p1 = rplot(tempc,C60[3s],n=3,showvar=false)

p2 = rplot(tempc,C[3s],λ_digits=3,showvar=false,n=3)

sp240 = ab(240ms,240ms,1,5,500Hz,6) |> normpower |> amplify(-10dB) |> spect
C240 = tempc(sp240)
